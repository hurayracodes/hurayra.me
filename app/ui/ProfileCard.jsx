...
const ProfileCardComponent = ({
  ...
  showUserInfo = true,
  onContactClick,
  // Naye props - effects control
  reduceShine = true,  // Default true karo taake image normal rahe
  reduceGlare = true   // Default true karo taake image normal rahe
}) => {
  ...
  const handleContactClick = useCallback(() => {
    if (onContactClick) {
      ...
      window.open('mailto:abuhuraira3912l@gmail.com', '_blank');
    }
  }, [onContactClick]);

  // Modified shine style with reduced effects
  const shineStyle = {
    maskImage: 'var(--icon)',
    maskMode: 'luminance',
    maskRepeat: 'repeat',
    ...
    filter: reduceShine ? 'brightness(0.9) contrast(1.1)' : 'brightness(0.66) contrast(1.33) saturate(0.33) opacity(0.5)',
    animation: reduceShine ? 'none' : 'pc-holo-bg 18s linear infinite',
    mixBlendMode: reduceShine ? 'normal' : 'color-dodge',
    '--space': '5%',
    '--angle': '-45deg',
    ...
    backgroundImage: reduceShine ? 'none' : `
    ...
    pointerEvents: 'none',
    opacity: reduceShine ? 0.3 : 1 // Kam opacity for subtle effect
  };

  // Modified glare style
  const glareStyle = {
    transform: 'translate3d(0, 0, 1.1px)',
    overflow: 'hidden',
    backgroundImage: reduceGlare ? 'none' : `radial-gradient(
      farthest-corner circle at var(--pointer-x) var(--pointer-y),
      hsl(248, 25%, 80%) 12%,
      hsla(207, 40%, 30%, 0.8) 90%
    )`,
    mixBlendMode: reduceGlare ? 'normal' : 'overlay',
    filter: reduceGlare ? 'none' : 'brightness(0.8) contrast(1.2)',
    zIndex: 4,
    gridArea: '1 / -1',
    borderRadius: cardRadius,
    pointerEvents: 'none',
    opacity: reduceGlare ? 0.2 : 1
  };

  // Image style - ensure it stays normal
  const imageContainerStyle = {
    mixBlendMode: 'normal', // Change from luminosity to normal
    ...
    >
      {behindGlowEnabled && (
        <div
          ...
          style={{
            ...
            opacity: reduceShine ? 'calc(0.3 * var(--card-opacity))' : 'calc(0.8 * var(--card-opacity))'
          }}
        />
      )}
      <div ref={shellRef} className="relative z-[1] group">
        ...
        >
          ...
          >
            {/* Shine layer - only if not reduced */}
            {!reduceShine && <div style={shineStyle} />}

            {/* Glare layer - only if not reduced */}
            {!reduceGlare && <div style={glareStyle} />}

            {/* Avatar content - Fixed for normal image display */}
            <div
              className="overflow-visible backface-hidden"
              style={imageContainerStyle}
            >
              <div className="w-full absolute left-1/2 bottom-[-1px] backface-hidden will-change-transform transition-transform duration-[120ms] ease-out"
                style={{
